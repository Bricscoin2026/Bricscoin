...
Each message costs a real BRICS fee (burned) making it a true on-chain transaction.
"""
...
CHAT_FEE = 0.000005  # Same as TRANSACTION_FEE - burned
...
async def send_message(msg: ChatMessage):
    """Send a PQC-signed encrypted message on-chain. Costs 0.000005 BRICS (burned)."""
    ...
    if balance < CHAT_FEE:
        raise HTTPException(
            status_code=400,
            detail=f"Insufficient balance for chat fee. Need: {CHAT_FEE} BRICS. Available: {balance}"
        )
    ...
    tx_id = hashlib.sha256(
        f"chat:{msg.sender_address}:{msg.content_hash}:{ts}".encode()
    ).hexdigest()
...
async def get_global_feed(limit: int = 10):
    messages = await db.chat_messages.find(
        ...
        {"_id": 0, "id": 1, "sender_address": 1, "recipient_address": 1,
         "encrypted_content": 1, "timestamp": 1, "block_height": 1, "pqc_verified": 1, "fee": 1, "tx_id": 1}
...
async def get_chat_stats():
    ...
    total_burned = total_messages * CHAT_FEE
