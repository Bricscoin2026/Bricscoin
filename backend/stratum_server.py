...
async def get_network_difficulty() -> int:
    ...
    if current_index > 0 and current_index % adjustment_interval == 0:
        last_blocks = (
            ...
            .sort("index", -1)
            .limit(adjustment_interval + 1)
            .to_list(adjustment_interval + 1)
        )
        ...
        if len(last_blocks) >= 2:
            ...
            try:
                first_time = datetime.fromisoformat(
                    last_blocks[0]["timestamp"].replace("Z", "+00:00")
                )
                last_time = datetime.fromisoformat(
                    last_blocks[-1]["timestamp"].replace("Z", "+00:00")
                )
...
def create_coinbase_tx(
    height: int, reward: int, miner_addr: str, extranonce1: str, extranonce2_size: int
) -> tuple:
...
def create_stratum_job(
    template: dict, miner_address: str, extranonce1: str = "00000000", extranonce2_size: int = 4
) -> dict:
    global job_counter, job_cache
    job_counter += 1
    job_id = format(job_counter, 'x')
    coinb1, coinb2 = create_coinbase_tx(
        template['index'], template['reward'], miner_address, extranonce1, extranonce2_size
    )
...
async def verify_share(
    ...
    try:
        ...
        # Stesso max_target del backend (check_difficulty in server.py)
...
class StratumMiner:
    ...
    async def handle_submit(self, msg_id, params):
        if not self.authorized:
            ...
            return
        try:
            # Alcuni miner possono mandare parametri extra: prendiamo solo i primi 5
            if len(params) < 5:
                self.respond(msg_id, False, [24, "Invalid submit params", None])
                return
    ...
    async def save_block(self, job, nonce, block_hash):
        ...
        logger.info(
            f"✅ Block #{template['index']} saved! Miner: {miner_address}, Reward: {reward_amount} BRICS"
        )
...
class StratumServer:
    ...
    async def start(self):
        ...
        logger.info(
            f"BricsCoin Stratum Server listening on {STRATUM_HOST}:{STRATUM_PORT}"
        )
