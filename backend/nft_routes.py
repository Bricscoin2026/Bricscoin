...
The blockchain records WHO signed WHAT and WHEN — immutably.
"""
...
async def mint_certificate(cert: MintCertificate):
    ...
    try:
        is_valid = hybrid_verify(sig_data, cert.ecdsa_signature, cert.dilithium_signature, cert.ecdsa_public_key, cert.dilithium_public_key)
        if not is_valid:
    ...
    certificate = {
        ...
        "transfer_history": [
            {"from": cert.creator_address, "to": cert.recipient_address or cert.creator_address, "timestamp": now, "type": "mint"}
        ],
        "revoked": False,
    }
...
async def list_certificates(limit: int = 50, cert_type: Optional[str] = None):
    ...
    certs = await db.nft_certificates.find(query, {"_id": 0}).sort("created_at", -1).limit(min(limit, 200)).to_list(min(limit, 200))
...
async def verify_certificate(cert_id: str):
    ...
    cert = await db.nft_certificates.find_one({"id": cert_id}, {"_id": 0})
    if not cert:
        return {"valid": False, "certificate_id": cert_id, "reason": "Certificate not found on the blockchain"}
    ...
    return {
        ...
        "title": cert["title"],
        "description": cert["description"],
        "certificate_type": cert.get("display_type", cert["certificate_type"]),
...
async def get_certificates_by_owner(address: str, limit: int = 50):
    ...
    certs = await db.nft_certificates.find({"owner_address": address, "revoked": {"$ne": True}}, {"_id": 0}).sort("created_at", -1).limit(min(limit, 200)).to_list(min(limit, 200))
...
async def get_certificates_by_creator(address: str, limit: int = 50):
    ...
    certs = await db.nft_certificates.find({"creator_address": address}, {"_id": 0}).sort("created_at", -1).limit(min(limit, 200)).to_list(min(limit, 200))
...
async def transfer_certificate(transfer: TransferCertificate):
    ...
    try:
        is_valid = hybrid_verify(sig_data, transfer.ecdsa_signature, transfer.dilithium_signature, transfer.ecdsa_public_key, transfer.dilithium_public_key)
        if not is_valid:
            raise HTTPException(status_code=400, detail="Invalid PQC signature")
    except Exception as e:
        raise HTTPException(status_code=400, detail=f"Signature verification failed: {str(e)}")
    ...
    await db.nft_certificates.update_one(
        ...
        {"$set": {"owner_address": transfer.to_address}, "$push": {"transfer_history": {"from": transfer.from_address, "to": transfer.to_address, "timestamp": now, "type": "transfer"}}}
    )
    return {"certificate_id": transfer.certificate_id, "new_owner": transfer.to_address, "previous_owner": transfer.from_address, "timestamp": now}
...
async def get_nft_stats():
    ...
    pipeline = [{"$match": {"revoked": {"$ne": True}}}, {"$group": {"_id": "$certificate_type", "count": {"$sum": 1}}}, {"$sort": {"count": -1}}]
    type_counts = {}
