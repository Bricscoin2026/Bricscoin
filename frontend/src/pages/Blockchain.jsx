...
/* ========== MINING SECTION ========== */
function MiningSection({ stats, minerStats }) {
  const [walletAddress, setWalletAddress] = useState("");
  useEffect(() => {
    const saved = localStorage.getItem('bricscoin_web_wallet');
    if (saved) { try { setWalletAddress(JSON.parse(saved).address || ""); } catch {} }
  }, []);
    ...
    <div className="space-y-5">
      ...
      <Card className="bg-red-500/10 border-red-500/30">
        <CardContent className="p-4 flex items-start gap-3">
          ...
          <div>
            <h4 className="font-bold text-red-400 text-sm">ASIC Hardware Required</h4>
            <p className="text-xs text-muted-foreground mt-1">BricsCoin uses SHA-256 PoW. Only ASIC miners (Bitaxe, Antminer, Whatsminer) are supported. CPU/GPU/phone mining is not possible.</p>
          </div>
        </CardContent>
      </Card>
      ...
      <div className="grid grid-cols-3 md:grid-cols-6 gap-3">
        {[
          { v: minerStats?.active_miners ?? "-", l: "Active Miners", c: "text-green-400" },
          { v: stats?.current_difficulty?.toLocaleString() || "-", l: "Difficulty" },
          { v: `${stats?.current_reward || 50}`, l: "Block Reward" },
          { v: stats?.total_blocks || 0, l: "Total Blocks" },
          { v: "SHA-256", l: "Algorithm" },
          { v: stats?.hashrate_from_shares > 0 ? formatHashrate(stats.hashrate_from_shares) : formatHashrate(stats?.hashrate_estimate), l: "Hashrate" },
        ].map(s => (
          <Card key={s.l} className="bg-card border-white/10">
            <CardContent className="p-3 text-center">
              <p className={`text-lg font-bold ${s.c || "text-primary"}`}>{s.v}</p>
      ...
      <Card className="bg-gradient-to-br from-primary/15 to-primary/5 border-primary/30">
        <CardHeader className="pb-2"><CardTitle className="text-sm flex items-center gap-2"><Cpu className="w-4 h-4 text-primary"/>Stratum Configuration</CardTitle></CardHeader>
        <CardContent className="space-y-2 font-mono text-xs">
          {[
            { l: "Pool URL", v: "stratum+tcp://stratum.bricscoin26.org:3333", c: "text-primary font-bold" },
            { l: "Alternative IP", v: "stratum+tcp://5.161.254.163:3333", c: "text-yellow-400" },
            { l: "Server", v: "stratum.bricscoin26.org" },
            { l: "Port", v: "3333" },
            { l: "Username", v: walletAddress || "YOUR_BRICS_WALLET_ADDRESS", c: "text-green-400" },
            { l: "Password", v: "x" },
          ].map(r => (
            <div key={r.l} className="flex items-center justify-between p-3 bg-black/40 rounded border border-white/10">
              <span className="text-muted-foreground">{r.l}:</span>
              <div className="flex items-center gap-2">
                <code className={r.c || "text-white"}>{r.v}</code>
                <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => copy(r.v)}><Copy className="w-3 h-3"/></Button>
              </div>
            </div>
          ))}
        </CardContent>
      ...
      <div className="grid md:grid-cols-2 gap-4">
        {[
          { title: "Bitaxe Setup", icon: Pickaxe, color: "text-orange-500", steps: ["Open Bitaxe web dashboard", "Settings → Pool", "Hostname: stratum.bricscoin26.org", "Port: 3333", "User: Your BRICS address", "Password: x", "Save & Restart"] },
          { title: "NerdMiner Setup", icon: Cpu, color: "text-purple-500", steps: ["Access NerdMiner web interface", "Settings → Mining Pool", "Pool: stratum.bricscoin26.org", "Port: 3333", "Address: Your BRICS address", "Save and restart"] },
        ].map(g => (
          <Card key={g.title} className="bg-card border-white/10">
            <CardHeader className="pb-2"><CardTitle className="text-sm flex items-center gap-2"><g.icon className={`w-4 h-4 ${g.color}`}/>{g.title}</CardTitle></CardHeader>
            <CardContent><ol className="text-xs text-muted-foreground space-y-1.5 list-decimal list-inside">{g.steps.map((s, i) => <li key={i}>{s}</li>)}</ol></CardContent>
          </Card>
        ))}
      </div>
      ...
      {/* Supported Miners */}
      <Card className="bg-card border-white/10">
        <CardHeader className="pb-2"><CardTitle className="text-sm">Supported ASIC Miners</CardTitle></CardHeader>
        <CardContent>
          <div className="grid grid-cols-4 md:grid-cols-8 gap-2">
            {["Bitaxe", "NerdMiner", "Antminer S9/S19", "Whatsminer", "AvalonMiner", "Canaan", "Innosilicon", "Ebang"].map(m => (
              <div key={m} className="p-2 bg-white/5 rounded text-center text-xs">{m}</div>
            ))}
          </div>
        </CardContent>
      </Card>
