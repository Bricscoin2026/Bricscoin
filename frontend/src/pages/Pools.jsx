...
export default function Pools() {
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <Card className="bg-card border-white/10">
          <CardContent className="p-4 text-center">
            ...
            <p className="text-xs text-muted-foreground">Fee Pool Ufficiale</p>
          </CardContent>
        </Card>
      </div>
          ...
          <Card 
            ...
            className={`bg-gradient-to-br ${
              pool.featured 
                ...
                : "from-card to-card/50 border-white/10"
            }`}
              ...
              <div className="flex items-center justify-between flex-wrap gap-2">
                <div className="flex items-center gap-3">
                  <div className={`w-3 h-3 rounded-full ${
                    pool.status === "online" ? "bg-green-500 animate-pulse" : "bg-red-500"
                  }`} />
                ...
                <div className="flex items-center gap-2">
                  <span className={`text-sm px-3 py-1 rounded-full ${
                    ...
                    {pool.status === "online" ? "Online" : "Offline"}
                  </span>
                </div>
              </div>
            </CardHeader>
            <CardContent className="p-6 space-y-6">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                <div className="bg-black/20 rounded-lg p-3 text-center">
                  ...
                  <p className="font-bold text-primary">{pool.fee}</p>
                </div>
                <div className="bg-black/20 rounded-lg p-3 text-center">
                  <p className="text-xs text-muted-foreground mb-1">Min Payout</p>
                  <p className="font-bold text-primary">{pool.minPayout}</p>
                </div>
                <div className="bg-black/20 rounded-lg p-3 text-center">
                  <p className="text-xs text-muted-foreground mb-1">Location</p>
                  <p className="font-bold text-primary text-sm">{pool.location}</p>
                </div>
                <div className="bg-black/20 rounded-lg p-3 text-center">
                  <p className="text-xs text-muted-foreground mb-1">Protocol</p>
                  <p className="font-bold text-primary">Stratum V1</p>
                </div>
              </div>
              ...
              <div className="space-y-3">
                ...
                <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 p-4 bg-black/40 rounded-lg border border-primary/20">
                  ...
                  <Button 
                    variant="outline" 
                    size="sm" 
                    onClick={() => copyToClipboard(pool.url)}
                    className="shrink-0"
      ...
      <Card className="bg-card border-white/10">
        <CardHeader>
          <CardTitle className="font-heading">Hardware Supportato</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {[
              { name: "Bitaxe", hashrate: "~1.2 TH/s", type: "ASIC" },
              { name: "NerdQaxe", hashrate: "~4.8 TH/s", type: "ASIC" },
              { name: "NerdMiner", hashrate: "~350 kH/s", type: "ESP32" },
              { name: "Antminer S19", hashrate: "~95 TH/s", type: "ASIC" },
              { name: "Antminer S21", hashrate: "~200 TH/s", type: "ASIC" },
              { name: "Whatsminer M50", hashrate: "~120 TH/s", type: "ASIC" },
            ].map((hw) => (
              <div key={hw.name} className="flex items-center justify-between p-3 bg-black/20 rounded-lg">
                <div>
                  <p className="font-semibold">{hw.name}</p>
                  <p className="text-xs text-muted-foreground">{hw.type}</p>
                </div>
                <span className="text-primary font-mono text-sm">{hw.hashrate}</span>
              </div>
            ))}
          </div>
        </CardContent>
      ...
      <Card className="bg-gradient-to-r from-primary/10 to-transparent border-primary/20">
        <CardContent className="p-6 flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <a 
            href="https://twitter.com/bricscoin" 
            target="_blank" 
            rel="noopener noreferrer"
          >
