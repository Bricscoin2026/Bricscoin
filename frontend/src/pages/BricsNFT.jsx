...
const CERT_TYPES = [
  { value: "diploma", label: "Diploma / Degree", icon: "🎓" },
  { value: "property", label: "Property Deed", icon: "🏠" },
  { value: "authenticity", label: "Authenticity Certificate", icon: "✅" },
  { value: "professional", label: "Professional License", icon: "💼" },
  { value: "membership", label: "Membership", icon: "🪪" },
  { value: "award", label: "Award / Achievement", icon: "🏆" },
...
function CertTypeIcon({ type }) {
  const found = CERT_TYPES.find(t => t.value === type);
  return <span className="text-lg">{found?.icon || "📝"}</span>;
}
...
export default function BricsNFT() {
  ...
  const handleMint = async () => {
    ...
    if (!wallet) {
      toast.error("PQC Wallet required. Load a wallet first.");
      return;
    }
    ...
    try {
      ...
      toast.success(`Certificate ${res.data.certificate.id} minted!`);
      setTitle("");
      setDescription("");
      setRecipient("");
      loadCertificates();
      ...
      toast.error(err?.response?.data?.detail || "Failed to mint certificate");
    } finally {
      setMinting(false);
    }
  };
    ...
    <div className="space-y-8" data-testid="bricsnft-page">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <p className="text-muted-foreground">
          Quantum-Proof On-Chain Certificates — World's First PQC-Signed NFT System
        </p>
      </motion.div>
      ...
      <Card className="bg-primary/5 border-primary/20">
        <CardContent className="p-3 flex items-center gap-3">
          ...
          <div>
            <p className="text-sm"><span className="font-bold text-primary">Mint Fee: 0.00001 BRICS</span> per certificate (burned)</p>
      ...
      <Tabs value={activeTab} onValueChange={(v) => setSearchParams({ tab: v })} className="space-y-6">
        <TabsList className="bg-card border border-white/10">
          ...
          <TabsTrigger value="mint" data-testid="tab-mint">
            <FileCheck className="w-4 h-4 mr-2" />Mint
          </TabsTrigger>
          <TabsTrigger value="verify" data-testid="tab-verify">
            <Fingerprint className="w-4 h-4 mr-2" />Verify
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="gallery" className="space-y-4">
          ...
          {loading ? (
            <div className="text-center py-12">
              ...
              <p className="text-sm text-muted-foreground mt-2">Loading certificates...</p>
            </div>
                    ...
                    <CardContent className="p-4 space-y-3">
                      <div className="flex items-start justify-between">
                        <div className="flex items-center gap-2">
                          <CertTypeIcon type={cert.certificate_type} />
                      ...
                      <div className="flex justify-between items-center text-xs text-muted-foreground pt-2 border-t border-white/5">
                        <span className="font-mono">{truncateAddress(cert.creator_address)}</span>
                ...
                {showWalletPicker && pqcWallets.length > 0 ? (
                  <div className="space-y-3">
                    <h2 className="text-sm font-heading font-bold flex items-center gap-2">
                      ...
                      Select PQC Wallet to Mint Certificates
                    </h2>
                  ...
                  <div className="flex flex-col sm:flex-row items-center gap-4">
                    ...
                    <div className="flex-1 text-center sm:text-left">
                      <h2 className="text-sm font-heading font-bold">PQC Wallet Required to Mint</h2>
                    ...
                    <div className="flex gap-2 flex-shrink-0">
                      <Button className="gold-button" size="sm"
                        onClick={async () => {
                          try {
                            ...
                            setWalletLoaded(true);
                            toast.success("PQC Wallet created! Send BRICS to it before minting.");
                          } catch (err) {
                      ...
                      <Button variant="outline" size="sm" onClick={() => window.location.href = "/wallet"} data-testid="go-wallet-btn">
                        Use Existing
                      </Button>
                    </div>
                  </div>
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <Input value={title} onChange={e => setTitle(e.target.value)}
                        placeholder="e.g. Bachelor's Degree in Computer Science" className="mt-1" data-testid="nft-title" />
                    </div>
                    <div>
                      <Label className="text-xs text-muted-foreground">Description</Label>
                      <textarea value={description} onChange={e => setDescription(e.target.value)}
                        placeholder="Detailed description of the certificate..."
                        className="w-full min-h-[120px] rounded border border-white/10 bg-background p-3 text-sm resize-none focus:outline-none focus:ring-1 focus:ring-primary"
                    ...
                    <Button className="gold-button w-full" onClick={handleMint} disabled={minting} data-testid="mint-btn">
                      {minting ? (
                        ...
                        <><Award className="w-4 h-4 mr-2" />Mint Certificate (0.00001 BRICS)</>
                      )}
                    </Button>
                      ...
                      {verifyResult.valid && (
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                          ...
                          <div>
                            ...
                            <Badge variant="outline">{verifyResult.certificate_type}</Badge>
                          </div>
                          <div>
                            <p className="text-xs text-muted-foreground">Issuer</p>
                            <p className="font-mono text-xs">{truncateAddress(verifyResult.creator)}</p>
                          </div>
                          <div>
                            <p className="text-xs text-muted-foreground">Owner</p>
                            <p className="font-mono text-xs">{truncateAddress(verifyResult.owner)}</p>
                          </div>
                          <div>
                            <p className="text-xs text-muted-foreground">Block</p>
                            <p>#{verifyResult.block_height}</p>
                          </div>
                          <div>
                            <p className="text-xs text-muted-foreground">Created</p>
                            <p className="text-xs">{formatTime(verifyResult.created_at)}</p>
                          </div>
                          <div>
                            <p className="text-xs text-muted-foreground">Content Hash (SHA-256)</p>
                            <p className="font-mono text-xs truncate">{verifyResult.content_hash}</p>
                          </div>
                          <div>
          ...
          {selectedCert && (
            <div className="space-y-4">
              <div className="flex items-center gap-2 mb-2">
                <CertTypeIcon type={selectedCert.certificate_type} />
                <Badge variant="outline">{selectedCert.certificate_type}</Badge>
                {selectedCert.pqc_verified && (
                  <Badge variant="outline" className="border-green-500/50 text-green-400">PQC Verified</Badge>
                )}
              </div>
              <div>
                <p className="text-xs text-muted-foreground">Title</p>
                <p className="font-heading font-bold">{selectedCert.title}</p>
              </div>
              <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <p className="text-xs text-muted-foreground">Issuer</p>
                  <p className="font-mono text-xs break-all">{selectedCert.creator_address}</p>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground">Owner</p>
                  <p className="font-mono text-xs break-all">{selectedCert.owner_address}</p>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <p className="text-xs text-muted-foreground">Certificate ID</p>
                  <p className="font-mono text-xs text-primary">{selectedCert.id}</p>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground">Block</p>
                  <p>#{selectedCert.block_height}</p>
                </div>
              </div>
              <div>
                <p className="text-xs text-muted-foreground">Content Hash (SHA-256)</p>
                <p className="font-mono text-xs break-all">{selectedCert.content_hash}</p>
              </div>
              <div>
                <p className="text-xs text-muted-foreground">Transaction</p>
                <p className="font-mono text-xs break-all">{selectedCert.tx_id}</p>
              </div>
              <div>
                <p className="text-xs text-muted-foreground">Created</p>
                <p className="text-xs">{formatTime(selectedCert.created_at)}</p>
              </div>
              {selectedCert.transfer_history && selectedCert.transfer_history.length > 0 && (
                <div>
                  ...
                  <div className="space-y-1">
                    {selectedCert.transfer_history.map((h, i) => (
                      <div key={i} className="flex items-center gap-2 text-xs">
                        ...
                        <span className="font-mono">{truncateAddress(h.from)}</span>
                        <ArrowRightLeft className="w-3 h-3" />
                        <span className="font-mono">{truncateAddress(h.to)}</span>
                      </div>
                    ))}
                  </div>
                </div>
